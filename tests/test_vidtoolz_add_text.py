...
def test_create_parser():
    ...
    result = parser.parse_args(['video.mp4', 'hello', "-st", "3", "-et", "7", "-f", "100"])
...
def test_add_text_to_video():
    ...
    video_with_text, fps = add_text_to_video(TEST_VIDEO_FILE, 'Test text', 0, 5, 'center')
...
def test_add_text_to_video_invalid_position():
    ...
    video_with_text, fps = add_text_to_video(TEST_VIDEO_FILE, 'Test text', 0, 5, 'invalid-position')
    assert video_with_text is not None
    assert fps > 0

def test_write_file():
    # Test writing the video to a file
    video_with_text, fps = add_text_to_video(TEST_VIDEO_FILE, 'Test text', 2, 5, 'center')
...
def test_add_text_to_video_invalid_fontsize():
    ...
    with pytest.raises(SystemExit):
        add_text_to_video(TEST_VIDEO_FILE, 'Test text', 0, 5, 'center', fontsize='invalid')
